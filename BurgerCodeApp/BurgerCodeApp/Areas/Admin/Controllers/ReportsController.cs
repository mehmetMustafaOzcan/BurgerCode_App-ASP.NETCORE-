...
namespace BurgerCodeApp.Areas.Admin.Controllers
{
    ...
    public class ReportsController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            report.TotalEarning = Baskets.Where(x => x.Stage == BasketStage.Completed).Sum(x => x.TotalPrice);
            report.ExtraEarning = (decimal)_context.ExtraDetails.Include(e => e.Extra).Include(bd => bd.BasketDetail).Sum(e => (e.Extra.Price * e.BasketDetail.Quantity));
            report.MenuEarning = report.TotalEarning - report.ExtraEarning;
            var Menus = _context.BasketDetails.Include(x => x.Menu).GroupBy(x => x.MenuId).Select(m => new { TotalQuantity = m.Sum(s => s.Quantity), MenuName = m.Select(mn => mn.Menu.Name).FirstOrDefault() }).OrderByDescending(a => a.TotalQuantity);
        ...
        public IActionResult SalesReports(string report)
        {
            if (report=="extras")
            {
                var extras = _context.ExtraDetails.
                    ...
                    .GroupBy(g => g.ExtraId)
                    .Select(ex => 
                    new { Name = ex.Select(x => x.Extra.Name).FirstOrDefault(), 
                        Total = ex.Sum(e => (e.Extra.Price * e.BasketDetail.Quantity)) }).ToList();
            ...
            else if (report == "menus")
            {
                var Menus = _context.BasketDetails.
                    ...
                    .GroupBy(g => g.MenuId)
                    .Select(md => 
                    new { Name = md.Select(x => x.Menu.Name).FirstOrDefault(),
                        UnitPrice=md.Select(c=>c.Menu.Price).FirstOrDefault() ,
                        Total = md.Sum(m => (m.Menu.Price * m.Quantity)) }).ToList();
